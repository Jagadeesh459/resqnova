...
export function LiveMissionBoard({ fallback, onOpenMission }: { fallback: ReactNode; onOpenMission: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!user) return;
      const { data: team } = await supabase.from("rescue_teams").select("id").eq("manager_auth_id", user.id).maybeSingle();
      if (!team) return;
      setTeamId(team.id);
      const { data } = await supabase.from("rescue_missions").select("id,request_id,mission_status,latitude,longitude,citizen_requests:request_id(request_id,people_count,emergency_type,risk_level,eta)").eq("rescue_team_id", team.id).neq("mission_status", "completed").order("last_updated", { ascending: false });
  ...
  if (!teamId || missions.length === 0) return <>{fallback}</>;
